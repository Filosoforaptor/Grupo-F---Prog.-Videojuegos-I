class UI {
    constructor(juego) {
        ...
        // Hacer que la interfaz de texto se haga invisible cuando uno pasa el mouse por encima
        const elementosInterfaz = [this.score1, this.score2, this.score3, this.reloj, this.jabonesT, this.texto1, this.texto2, this.texto3, this.jabonIMG, this.fondoTimer];

        elementosInterfaz.forEach(elemento => {
            elemento.interactive = true;
            elemento.on('mouseover', () => {
                elemento.alpha = 0.5;
            });
            elemento.on('mouseout', () => {
                elemento.alpha = 1;
            });
        });
    ...
    triggerGameOver() {
        ...
        playMusic(canciones.GAME_OVER, 0.5);
    }
